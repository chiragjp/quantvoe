...
full_subset = full %>% filter(!is.na(LBDVIASI), !is.na(VIDRVA) & !is.na(VIDLVA))
...
full_subset_dep = full_subset %>% select(SEQN,VIDRVA,VIDLVA)
full_subset_ind = full_subset %>% select(-c(VIDRVA,VIDLVA))

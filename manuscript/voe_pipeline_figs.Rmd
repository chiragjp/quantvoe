...
benchmarkdata = read.csv('jobtimedata.csv',header=F,sep=',') %>% select(V1,V2,V3) %>% rename(jobsize=V2,elapsed_time=V3)
benchmarkdata$elapsed_time = as.numeric(gsub('S','',lubridate::seconds(hms(benchmarkdata$elapsed_time))))
benchmarkdata$jobsize = as.numeric(gsub('G','',benchmarkdata$jobsize))

colnames(benchmarkdata)=c('Number of vibrations','jobsize','elapsed_time')

ggplot(data=benchmarkdata,aes(x=log(`Number of vibrations`),y=(elapsed_time))) + geom_point() + ggtitle('Runtime benchmarking') + xlab('log(Number of vibrations)') + ylab('Log(Seconds)')
ggsave('figures/jobtimes_multicore.pdf',width=7,height=5)

ggplot(data=benchmarkdata,aes(x=log(`Number of vibrations`),y=(jobsize))) + geom_point() + ggtitle('Memory usage')+ xlab('log(Number of vibrations)') + ylab('Log(RAM used, GB)')
...
files=list.files()[grep('benchmarking',list.files())]
...
for(p in phenotypes){
  ...
  for(f in files_sub){
    print(f)
    data = readRDS(f)
    name = strsplit(strsplit(f,'_')[[1]][5],'\\.')[[1]][1]
  ...
  confounder_analysis_list = confounder_analysis_list[, c(1,5,8,2,6,9,11,3,7,10,12,4)]
  confounder_analysis_list = cor(confounder_analysis_list,use='complete.obs')
  write.csv(confounder_analysis_list,paste('../figures/benchmarking_corplot_data.csv',sep=''))
  pdf(paste('../figures/benchmarking_corplot_data.pdf',sep=''),width=5,height=5)
  pheatmap(confounder_analysis_list,cluster_rows=FALSE,cluster_cols=FALSE,breaks=breaksList,legend = FALSE,color=cols(14))
  dev.off()
}

num_obs_pe_var = do.call('rbind',confounder_presence_data)  %>% melt %>% mutate(num=strsplit(as.character(Var1),'_') %>% map_chr(5) %>% strsplit('\\.') %>% map_chr(1)) %>% select(-Var1,-Var2) %>% group_by(num) %>% summarize(mean=mean(value),sd=sd(value)) %>% mutate(num=as.numeric(num)) %>% arrange(num)
...
vision_carrots = full_output %>% filter(dependent_feature == 'VIDLVA',independent_feature == 'FFQ0032') %>% select_if(~ length(unique(.)) > 1) %>% mutate(sumval = rowSums(full_output %>% filter(dependent_feature == 'VIDLVA',independent_feature == 'FFQ0032') %>% select_if(~ length(unique(.)) > 1) %>% select(-estimate,-std.error,-full_fits,-p.value,-statistic))) %>% arrange(sumval) %>% select(full_fits,estimate,p.value)
...
max_x_val = max(abs(full_output %>% filter(term=='FFQ0032',dependent_feature=='VIDLVA') %>% select(estimate) %>% range))
...
ggplot(data = foo, aes(x = estimate, y = -log10(p.value))) + geom_point() + xlim(-1*max_x_val,max_x_val) + ylim(0,max_y_val)
ggsave(paste('../figures/','FFQ0032','_BLANK_','VIDLVA','.pdf',sep=''),width=3,height=3)
...
data = readRDS('vitd_median_30890_quantvoe_output.rds')
...
vitd_covid = vibration_data$data %>% filter(dependent_feature == 'covid_positive_status', p.value<0.1) %>% select_if(~ length(unique(.)) > 1) %>% mutate(sumval = rowSums(vibration_data$data %>% filter(dependent_feature == 'covid_positive_status',p.value<0.1) %>% select_if(~ length(unique(.)) > 1) %>% select(-estimate,-std.error,-full_fits,-p.value,-statistic))) %>% arrange(sumval) %>% select(full_fits,estimate,p.value)
data.frame(vitd_covid %>% filter(estimate>0) %>% filter(p.value<0.05))[1,1]
data.frame(vitd_covid %>% filter(estimate<0))[1,1] 

foo = bind_rows(data.frame(vitd_covid %>% filter(estimate<0))[1,1] [[1]] %>% filter(term=='median_30890') %>% select(estimate,p.value),data.frame(vitd_covid %>% filter(estimate>0) %>% filter(p.value<0.05))[1,1][[1]] %>% filter(term=='median_30890') %>% select(estimate,p.value))

ggplot(data = foo, aes(x = estimate, y = -log10(p.value))) + geom_point() + xlim(-1*max_x_val,max_x_val) + ylim(0,max_y_val)
ggsave(paste('../figures/','covid','_BLANK_','median_30890','.pdf',sep=''),width=3,height=3)

...
Because we are using a regression analysis to look at the impact of adjusting variables, you are subject to any of the issues that could come with mixed effect (or plain linear) regression. Pay attention to the output to look for convergence warnings. Additionally, given that we provide the full model output in addition to summarized output, we recommend that you also plot regression validation plots to confirm that your specific data is not violating critically any of the core assumptions of linear regression (e.g. homoscedasticity). If you still have questions, feel free to contact the authors or look at the package vignette.
